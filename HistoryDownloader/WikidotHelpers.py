...
def CannonicizeString(name):
    out = []    # An array of characters in which we'll build the cannonical name
    inAlpha = False # We're not in a span of alphabetic character
    inJunk = False  # We're not in a span of junk characters. (Junk characters are those which get reduced to a hyphen in Wikidot names.)
    for c in name:
        if c.isalnum() or c == ':':     # ":", the category separator, is an honorary alphanumeric
            ...
            inJunk = True
            inAlpha = False
    # Create a cannonical name string from the character array
    canname=''.join(out)
...
def Cannonicize(pageNameZip):
    ...
    # Handle the common case of no category
    if len(splitName) == 1:
